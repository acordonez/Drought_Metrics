...
python ${CMEC_CODE_DIR}/drought_metrics.py ${CMEC_CONFIG_DIR}/cmec.yaml >> ${CMEC_WK_DIR}/drought_metrics_log.txt

# Make cmec outputs if drought metrics succeeds
if [[ $? = 0 ]]; then
    echo "Creating CMEC output"
    python ${CMEC_CODE_DIR}/dm_cmec_outputs.py ${CMEC_CONFIG_DIR}/cmec.yaml
